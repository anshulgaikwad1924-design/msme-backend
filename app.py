...
def forecast():
    ...
    if file.filename == "" or not allowed_file(file.filename):
        return jsonify({"error": "Invalid file. Upload .csv or .xlsx"}), 400
    try:
        ...
        return jsonify({"error": "Forecast failed. Please check your file format and try again."}), 500
...
def inventory_status():
    ...
    try:
        result = analyze_inventory(data["products"])
        return jsonify(result), 200
    except Exception as e:
        app.logger.exception(e)
        return jsonify({"error": "Inventory analysis failed. Please try again."}), 500
...
def revenue_risk():
    ...
    if not data or "products" not in data:
        return jsonify({"error": "Send JSON with products array"}), 400
    try:
        result = calculate_revenue_risk(data["products"])
        return jsonify(result), 200
    except Exception as e:
        app.logger.exception(e)
        return jsonify({"error": "Revenue risk calculation failed. Please try again."}), 500
...
def whatsapp_alert():
    ...
    if not re.match(r"^\+\d{10,15}$", phone):
        return jsonify({"error": "Invalid phone number. Use format: +919876543210"}), 400
    try:
        result = send_whatsapp_alert(
            ...
            store_name=data.get("store_name", "MSME Store"),
        )
        return jsonify(result), 200
    except Exception as e:
        app.logger.exception(e)
        return jsonify({"error": "WhatsApp alert failed. Check your Twilio credentials."}), 500
